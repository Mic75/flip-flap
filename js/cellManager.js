...
define( [ "text!../shaders/fsTexture.glsl",
  ...
  var manager = function( spec, my ){
     ...
     * Private Members
     */
    var that, cells, fontsTexture, colonTexture, gl, texShader, colShader, cellPageBuffers, frustDimensions, cellWidth,
    ...
    function initColonTexture(){
      ...
      context.beginPath();
      context.arc(64, 90, 10, 0, Math.PI*2);
      context.fill();
      context.arc(64, 166, 10, 0, Math.PI*2);
      context.fill();
      
      colonTexture = gl.createTexture();
      colonTexture.image = canvas;
      gl.bindTexture( gl.TEXTURE_2D, colonTexture );
      gl.texImage2D( gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, colonTexture.image );
    ...
    function initFontsTexture(){
      ...
      //colons (:) use for the clock display
      initColonTexture();
    }
    ...
    that.displayCells = displayCells;
    ...
    function displayClock(){
      ...
      for ( j = 0; j < spec.colCount; j++ ){
        cells.push( cell( {
          ...
          pos: [
            j * cellWidth + cellsBB.x.min + cellWidth / 2,
            cellsBB.y.min + cellHeight / 2,
            frustDimensions.depth / 2
          ],
          fontsTexture: j === 2 || j === 5 ? [colonTexture] : fontsTexture,
          graphics: spec.graphics,
          currentFontIndex: 0,
          name: "Cell0," + j
        } ) );
    ...
    that.displayClock = displayClock;
    ...
    that.testInitFontsTexture = function(){
      ...
      colonTexture.image.style.marginRight = "1px";
      document.body.appendChild( colonTexture.image );
  };
    return that;
  };
  return manager;
